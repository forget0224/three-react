...
export default function Sampler() {
  ...
  const sounds = [
    new Howl({ src: ['../audio/kick.wav'] }),
    new Howl({ src: ['../audio/snare.wav'] }),
    new Howl({ src: ['../audio/hihat.wav'] }),
    new Howl({ src: ['../audio/kick.wav'] }),
    new Howl({ src: ['../audio/snare.wav'] }),
    new Howl({ src: ['../audio/hihat.wav'] }),
    new Howl({ src: ['../audio/kick.wav'] }),
  ...
  const playSound = (index) => {
    console.log('Index being passed to playSound:', index)
    if (sounds[index]) {
      sounds[index].play()
    }
  }
  ...
  useEffect(() => {
    const init = () => {
      ...
      function createTextTexture(text) {
        ...
        const size = 256 // 設置紋理大小
        canvas.width = size
        canvas.height = size

        // 設置背景和字體顏色
        context.fillStyle = '#FFFFFF' // 背景顏色
        context.fillRect(0, 0, size, size)
        context.fillStyle = '#000000' // 字體顏色
      ...
      for (let i = 0; i < 4; i++) {
        for (let j = 0; j < 4; j++) {
          const buttonMaterial = new THREE.MeshStandardMaterial({
            map: createTextTexture(index.toString()), // 將 index 作為文字顯示
          })
          const button = new THREE.Mesh(buttonGeometry, buttonMaterial)
          button.rotation.x = Math.PI / 2
          // button.position.set(-3 + i * 2.5, -3 + j * 2.5, 1.2)
          button.position.set(-3 + j * 2.5, -3 + (3 - i) * 2.5, 1.2)
          buttonGroup.add(button)

          button.userData = { index } // 將按鈕的索引保存到 userData 中
          button.onClick = () => playSound(button.userData.index)
          index++
        }
      }
      ...
      const onMouseClick = (event) => {
        ...
        if (intersects.length > 0) {
          ...
          console.log(
            'Raycaster detected button with index:',
            clickedButton.userData.index,
          )

          // 播放對應的聲音
          playSound(clickedButton.userData.index)
        }
      }
